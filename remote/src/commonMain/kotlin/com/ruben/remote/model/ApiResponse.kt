...
sealed class ApiResponse<out SUCCESS, out ERROR> {
    ...
    object ErrorNoBody: ApiResponse<Nothing, Nothing>()
...
class ApiResponseSerializer<S, E>(
    ...
    override fun deserialize(decoder: Decoder): ApiResponse<S, E> {
        ...
        return when {
            (element is JsonObject && "errorCode" in element) -> ApiResponse.Error(decoder.json.decodeFromJsonElement(errorSerializer, element))
            (element is JsonObject || element is JsonArray) -> ApiResponse.Success(decoder.json.decodeFromJsonElement(successSerializer, element))
    ...
    override fun serialize(encoder: Encoder, value: ApiResponse<S, E>) {
        ...
        when (value) {
            ...
            is ApiResponse.SuccessNoBody, ApiResponse.ErrorNoBody, ApiResponse.UnknownError -> {
                //do nothing
            }
        }
    }
